...
cat /kafka/config/server.properties.template | sed \
  ...
  -e "s|{{ZOOKEEPER_SESSION_TIMEOUT_MS}}|${ZOOKEEPER_SESSION_TIMEOUT_MS:-7301000}|g" \
  -e "s|{{GROUP_MAX_SESSION_TIMEOUT_MS}}|${GROUP_MAX_SESSION_TIMEOUT_MS:-7300000}|g" \
  -e "s|{{SESSION_TIMEOUT_MS}}|${SESSION_TIMEOUT_MS:-300000}|g" \
  -e "s|{{LOG_RETENTION_MINS}}|${LOG_RETENTION_MINS:-30}|g" \
  -e "s|{{LOG_RETENTION_CHECK_INTERVAL_MS}}|${LOG_RETENTION_CHECK_INTERVAL_MS:-300000}|g" \
  -e "s|{{LOG_SEGMENT_BYTES}}|${LOG_SEGMENT_BYTES:-10737418}|g" \
